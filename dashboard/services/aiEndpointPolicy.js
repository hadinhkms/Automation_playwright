 ...
 * Which AI endpoints the dashboard server may call, and which key may travel with the call.
 *
 * Rule: the API key saved in .env only ever goes to the endpoint it was saved for. A browser
 * (or another web page) can name any baseURL, so the server never pairs its own key with an
 * endpoint chosen by the request.
 */
...
/** Origin with loopback aliases folded together, so localhost and 127.0.0.1 compare equal. */
...
/** The endpoint the server key belongs to: the saved Base URL, or 9Router when 9Router is the saved provider. */
...
/**
 * GET /api/ai/models: the key comes from the request header (personal scope) or from .env,
 * and the .env key is attached only when the target is the saved endpoint.
 */
...
/**
 * POST /api/ai/test-connection: a key typed by the user may be tested against any endpoint;
 * the saved .env key may only be tested against the endpoint it was saved for.
 */
...
/** Values written to .env must stay on one line; a newline would let a request add its own variables. */
...
/**
 * The dashboard listens on loopback only, so a legitimate AI settings request comes from the
 * dashboard page itself. Anything else is a cross-site request or a DNS-rebinding attempt.
 */
