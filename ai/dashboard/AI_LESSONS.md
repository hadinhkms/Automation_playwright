...
- Root cause:
  ...
  2. *CSS `[hidden]` Boolean Attribute Conflict*: Setting `panel.hidden = false` in JS does not remove the boolean attribute `hidden=""` in certain browser environments. When CSS specifies `.docs-subpanel[hidden] { display: none !important; }`, the panel stays hidden until `panel.removeAttribute('hidden')` is explicitly called. The reverse also happens (2026-09-26): any author `display` rule beats the browser's `[hidden]` style, so `.settings-card { display: grid }` kept `#settings-ai-status-alert` visible and its close button did nothing. A component toggled with `hidden` that also has a `display` rule needs its own `.x[hidden] { display: none; }`.
...
### 2026-09-26 — The Server AI Key Goes Only to the Endpoint It Was Saved For

- Area: AI settings routes (`dashboard/routes/aiRoutes.js`) and Settings → Cấu hình AI.
- Symptom: A 9Router key was hard-coded in `aiRoutes.js` and `app.js` and synced to both satellites. `GET /api/ai/models?baseURL=…&apiKey=…` put the key in the URL. Both that route and `POST /api/ai/test-connection` sent the `.env` key to whatever `baseURL` the request named, so any web page open in the same browser could make the dashboard send the key to its own host. `POST /api/ai/config` wrote values with newlines into `.env`, which let a request add its own variables. The QA services that call AI (`qaInferenceService` — including `/api/qa/scaffold/extract`, which used only the `.env` key with `payload.baseURL` — `qaRequirementAnalyzerService`, `qaConflictService`) paired the `.env` key with `clientConfig.baseURL` the same way.
- Root cause: The routes trusted the request to pick the endpoint, and paired the server key with it. The body parser accepts JSON sent as `text/plain`, so a cross-site form or `fetch` reached these routes without a CORS preflight.
- Correct pattern: `dashboard/services/aiEndpointPolicy.js`. The `.env` key is attached only when the target is the saved Base URL; `localhost` and `127.0.0.1` count as the same host. `/api/ai/models` accepts only 9Router on port 20128, official provider APIs, or the saved Base URL. A key the user types travels in the `X-AI-Config` header, never in the URL. `/api/ai/*` rejects cross-site requests (`Sec-Fetch-Site`, `Origin`) and non-loopback `Host` headers. `.env` values must be single-line. Services call `mayUseServerKey(requestedBaseURL, env)` before falling back to the `.env` key.
- Preventive rule: Never put a real key in source or URLs; `check:framework` now fails on `sk-…`, `AIza…` and GitHub tokens (`scripts/lib/secretScan.js`). A route that calls out with a stored credential must derive the destination from stored config, not from the request.
- Regression check: `node --test dashboard/services/aiEndpointPolicy.test.js scripts/lib/secretScan.test.js` `tests/dashboard-api/settings-ai-security.test.js` (6 of 9 fail against the previous `aiRoutes.js`) and `dashboard/services/aiKeyBinding.test.js` (4 of 8 fail against the previous services).
- Related files: `dashboard/services/aiEndpointPolicy.js`, `dashboard/routes/aiRoutes.js`, `dashboard/public/app.js`, `scripts/lib/secretScan.js`, `scripts/check-framework-structure.js`.
