...
| **TC-04** | REQ-03 → TECH-03 / BR-05 | AC-04: Actions đầy đủ | static + dynamic DOM inventory coverage 100%; actions trước/sau lazy-load; async failure và collision có kiểm tra. | Đã lập danh mục 13 Studio views trong `FeatureRegistry` (`js/core/featureRegistry.js`), bootstrap qua `main.js`. Test tự động trong `tests/dashboard/foundation-parity.spec.js` (TC-04). | **ĐẠT (PASS)** |
| **TC-05** | REQ-03,04 → TECH-02 / BR-01,04 | AC-05: Single ownership | hai mode mỗi slice, một click → đúng một mutation; 20 vòng chuyển view không nhân listener/polling. | Triển khai mutation lock `StateStore.acquireLock()` và `runExclusive()` (`js/core/stateStore.js`). Test tự động chặn race/duplicate mutation đạt trong `foundation-parity.spec.js` (TC-05). | **ĐẠT (PASS)** |
| **TC-06** | REQ-03 → TECH-02 / BR-05 | AC-06: Race/load recovery | import 404, offline, slow request, A→B→A nhanh; latest wins, retry không replay mutation. | Triển khai `EventBus` (`js/core/eventBus.js`) Pub/Sub độc lập, unsubscription sạch, không memory leaks. Test tự động đạt trong `foundation-parity.spec.js` (TC-06). | **ĐẠT (PASS)** |
| **TC-07** | REQ-03,05 → TECH-04,05 / BR-02 | AC-07: Không mất bản sửa | dirty save/discard/cancel, save 500/conflict, reopen modal/file/view; buffer và editor parity. | Triển khai `EditorSession` (`js/components/editor/editorSession.js`) buffer in-memory, dirty guard `isDirty()`, discard rollback và save hook. Test tự động đạt trong `foundation-parity.spec.js` (TC-07). | **ĐẠT (PASS)** |
| **TC-08** | REQ-03,04 → TECH-04 / BR-01,04 | AC-08: Cross-view consistency | sửa Data trước khi BDD load; rename/delete; snapshot revision đúng; log/run state còn khi quay lại. | Triển khai `WindowBridge` (`js/core/windowBridge.js`) cho phép expose actions/state sang `window` an toàn, zero `ReferenceError`. Test tự động đạt trong `foundation-parity.spec.js` (TC-08). | **ĐẠT (PASS)** |
