...
- [ ] 5.4 **Chặn — chưa sync.** `node scripts/pre-sync-drift.js` (chỉ đọc) báo cả 2 vệ tinh (`Vieclam24h-Automation_JS`, `Automation_Carthings`) có nội dung riêng mà Hub chưa từng có trong `tools/qa/lib/commands.js` và `tools/qa/lib/sources.js`: hỗ trợ nhiều TC/AC trong một title (`allTcIds`, `allAcIds`, `getAutomatedTcIds`, `getWipTcIds`). `npm run sync:satellites` sẽ xoá phần này. Cần quyết định trước khi sync: đưa ngược phần multi-TC lên Hub (khuyến nghị, vì PLAN-18 đổi cùng file `sources.js`) hoặc chuyển sang `core/local/`. 20 file còn lại chỉ là bản cũ của Hub, ghi đè an toàn.
  - **Cập nhật 2026-09-26 (chạy drift từng vệ tinh):** chỉ CarThings có multi-TC (`[TC-010 TC-011]` trong title, 10 spec dùng). Đã đưa lên Hub nguyên văn + áp cho `impact`, `filterDomain/filterTag`; `matrix` ghi cảnh báo khi không liệt kê được test. Chạy `tools/qa` của Hub trên CarThings (chỉ đọc): coverage 43 dòng khớp 100% với bản CarThings; Hub cũ sẽ làm lệch 18 dòng và sinh 31 finding sai. Vieclam24h có phần riêng khác: trường `line` + `distinctDecls` cho F-03, đã được `rowId` của Hub thay thế (test "1 TC bao phủ nhiều AC trên cùng một dòng"), ghi đè không mất hành vi. **Còn lại:** commit Hub (drift đọc lịch sử commit), chủ dự án đồng ý ghi đè `tools/` của Vieclam24h, và xử lý file chưa commit ở CarThings (`sync-satellites.js` chạy `git add -A` trong vệ tinh).
