...
> **Mã kế hoạch:** `PLAN-17`
> **Trạng thái:** `DRAFT v2 — ĐÃ REVIEW 2 LƯỢT + REVIEW CHÉO (kỹ thuật, UI/UX). CHỜ DUYỆT. Chưa implement hạng mục nào.`
...
> **Khuyến nghị:** G1 nên sửa ngay, không phụ thuộc việc duyệt plan (xem F0).
...
| R6 | Dữ liệu mẫu để đo chất lượng AI | **Dùng dự án vệ tinh CarThings** (`D:/_CarThings/Automation_Carthings`) | Có sẵn 7 REQ (`REQ-001`, `REQ-010`…`REQ-015`), 7 file test case + `traceability.md`, 7 lỗi test thật (`error-context.md`), 37 ảnh chụp. Bộ mẫu **nằm lại trong repo CarThings**, không copy vào framework (xem F6a) |
| R7 | Số người dùng / implement | **Chỉ 1 người** (chủ dự án) | Lộ trình tính tuần tự cho 1 người, có bản "gọn" để sớm có giá trị (Mục 6). Không cần danh tính/phân quyền; cấu hình cá nhân và server thực chất là một |
...
4. **Output có schema.** Sai schema → thử lại 1 lần → rơi về luật / báo lỗi theo nguyên tắc 2.
...
8. **Vòng đời chặt.** Mỗi lời gọi AI có timeout theo tác vụ, hủy được, gắn `AbortController` vào `disposers` của view; phản hồi về muộn (đã chuyển view/entity) bị bỏ qua.
...
| F1 | **AI Gateway** `core/ai/gateway/` | `callAi({ task, input, schema, clientConfig, signal })`. Cấu hình duy nhất từ Settings: cá nhân (`X-AI-Config`) → server (`.env`) → báo "Chưa cấu hình AI". Adapter chính OpenAI-compatible cho 9Router. Timeout theo tác vụ + `AbortSignal` + retry, kiểm schema, che secret, đọc `usage`. Nhận biết `ECONNREFUSED` → "9Router chưa chạy", 429 → "Hết hạn mức". **Chuyển toàn bộ ~17 lời gọi trong 7 module** (bảng kiểm kê kèm file:line đưa vào plan chi tiết). Test: chọn 9Router → 0 request tới googleapis/openai.com | M |
...
| F6 | Eval harness `core/ai/eval/` | Provider giả (mock) để test không tốn token; `npm run ai:eval -- --corpus=<đường dẫn>` in điểm từng tác vụ. Chỉ chứa **code + vài mẫu tổng hợp nhỏ**, không chứa dữ liệu dự án. Đặt trong `core/` vì `tests/` bị chặn sync; script `ai:eval` phải thêm vào danh sách script được merge sang vệ tinh (hiện chỉ merge `mp:*`) | M |
| F6a | **Bộ dữ liệu mẫu từ CarThings** (G6, R6) | Đặt tại `D:/_CarThings/Automation_Carthings/.tmp/ai-eval-corpus/` — **không copy vào framework** (nếu nằm trong framework, `sync:satellites` sẽ đẩy dữ liệu CarThings sang Vieclam24h). Gồm: (1) **REQ/TC**: 7 REQ + test case thật; tạo thêm bản sao có cài câu mơ hồ / AC mâu thuẫn / thiếu TC âm để đo BA-1, QA-1 (≥ 10 mẫu); (2) **Lỗi test**: 7 `error-context.md` thật + tạo thêm bằng **cố ý làm hỏng** spec CarThings (sai locator, timeout, API trả 500, assertion lệch, popup che) → ≥ 20 lỗi, gán nhãn tay (Lỗi sản phẩm / Lỗi test / Môi trường / Flaky). Chỉ chạy lỗi cố ý trên **môi trường test/staging**, không chạy trên production. **Loại khỏi mọi thứ gửi Gemini:** `data/CCCD/` (ảnh căn cước), ảnh avatar, tài khoản/mật khẩu trong `data/*.json` | M |
...
| F8 | Ngân sách token (G8, G10, R5) | Cửa sổ trượt **5 giờ**, hạn mức mặc định 1.000.000 (chỉnh trong Settings), lưu ở `/.tmp/ai-usage.json` (ghi atomic, chống 2 request ghi cùng lúc). Lấy `usage` thật; thiếu thì ước tính `ký tự / 4`; tính cả lời gọi lỗi/bị hủy. Ghi rõ "**ước tính** — 9Router dùng chung với công cụ khác"; nếu 9Router có API thống kê thì ưu tiên dùng. **Tái dùng pill `#agent-quota-pill`** ở header (sửa `role`, bỏ điều hướng kép, thêm "reset lúc HH:mm" vào title/aria) + card chi tiết trong Settings tái dùng `.agent-quota-bar-*`. **Một bảng ngưỡng duy nhất:** ≥80% cảnh báo, ≥95% chỉ cho tác vụ nhẹ, hết → rơi về luật. Mỗi tác vụ có trần token/lần; giới hạn chạy song song (mặc định 2). Chạy hàng loạt: ước tính trước + hỏi xác nhận | M |
...
| **QA-2** | Sinh Playwright spec từ TC | TC → spec dùng **đúng** POM, fixture, `actionRegistry`. Sinh xong chạy `check:framework` + chạy thử; lỗi → AI sửa ≤2 vòng; vẫn lỗi → nháp cho người. Không có luật thay thế | `core/ai/tasks`, generator | H | L | TB |
...
## 6. Lộ Trình (Option A; **1 người làm tuần tự** — R7)
